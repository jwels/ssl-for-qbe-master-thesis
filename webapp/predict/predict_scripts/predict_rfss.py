...
def predict_rfss(modelParams, sslThreshold):
    ...
    base_model = RandomForestClassifier(**modelParams)
    ...
    tree.plot_tree(model.base_estimator_.estimators_[0], feature_names=x_train.columns, impurity=False)
    ...
    where_conditions = ""
    for sub_tree in model.base_estimator_.estimators_:
        where_conditions = where_conditions + get_predicate(sub_tree, x_train.columns) + " OR "
    where_conditions = where_conditions.rsplit(' ', 1)[0]
