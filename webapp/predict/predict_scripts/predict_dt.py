...
def predict_dt(modelParams):
    ...
    # get the training data from the user input
    ...
    model = tree.DecisionTreeClassifier(**modelParams)
   ...
   # ----------------------------------------------------------------------------------------------------
    ...
    tree.plot_tree(model, feature_names=x_train.columns, impurity=False)
    ...
    where_conditions = get_predicate(model, x_train.columns)
