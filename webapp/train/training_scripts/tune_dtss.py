...
def hyperopt_dtss(model):
    ...
    trials = Trials()
    best = fmin(partial(tune, modelDBObject=model), space, trials=trials, algo=tpe.suggest, max_evals=model.maxEvals)
...
def tune(params, modelDBObject):
    ...
    base_model = tree.DecisionTreeClassifier(**params)
    ssl_base_model = tree.DecisionTreeClassifier(**params)
    model = SelfTrainingClassifier(ssl_base_model, threshold=modelDBObject.removeLowestCertaintyPercentage)
    ...
    if(len(f1_scores_val)==0 or len(f1_scores_test)==0):
        ...
        total_f1_score_test = statistics.median(f1_scores_test)
        total_f1_score_val = statistics.median(f1_scores_val)
